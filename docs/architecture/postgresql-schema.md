...
The repository includes ordered `.up.sql` and `.down.sql` files so the database can evolve deliberately. The next backend steps are to move more repository code, especially auth persistence, onto these canonical tables and to add optional telemetry enrichers on top of the new operational-state layer.
